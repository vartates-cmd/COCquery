...
> = {
  ...
  update: {
    label: "Duplicate — will update",
    className: "bg-blue-50 text-blue-800 ring-blue-600/20",
  },
  skip: {
    label: "Duplicate — will skip",
    ...
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6">
        ...
        <ul className="mt-3 space-y-1 text-sm text-emerald-900">
          <li>
            {commit.created} record{commit.created === 1 ? "" : "s"} created
          </li>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        ...
        <form action={previewAction} className="mt-4">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Up to {MAX_IMPORT_BYTES / 1024 / 1024} MB and{" "}
      ...
      {hasPlan && plan ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          ...
          <p className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-800">
            ...
            <strong>{plan.counts.update}</strong> updated ·{" "}
            <strong>{plan.counts.skip}</strong> skipped ·{" "}
            <strong>{plan.counts.error}</strong> with errors
          </p>
              ...
              {(["skip", "update"] as const).map((option) => (
                ...
                >
                  <input
                    ...
                    onChange={() => {
                      setStrategy(option);
                      // Dispatched with an explicitly built payload rather than
                      // by re-submitting the form. Submitting would read the
                      // hidden strategy input, which React has not re-rendered
                      // yet at this point — the preview would silently be built
                      // from the previous choice.
                      const payload = new FormData();
                      payload.set("fileName", fileName);
                      payload.set("fileText", fileText);
                      payload.set("strategy", option);
                      previewAction(payload);
                    }}
                  />
                  {option === "skip"
                    ? "Leave it unchanged"
                    : "Update it from the file"}
                </label>
              ))}
            </div>
          </fieldset>
                ...
                {plan.rows.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.line}
                      className={
                        row.state === "error" ? "bg-red-50/40" : undefined
                      }
                    >
      ...
      {hasPlan && plan ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Nothing has been written yet. Rows with errors are never written —
